...
include ("functions/baglanti.php");
		if(isset($_POST["ggonder"])){
			$baslik =mysql_real_escape_string($_POST["gbaslik"]);
			$icerik =mysql_real_escape_string($_POST["gicerik"]);
			$level =sGet('name');
			$tarih =date("d-m-Y");
...
<?php
				...
				$idd=$_POST["gcheck"];
					foreach($idd as $gsil){
						$temizle=mysql_query("delete from gduyuru where ID='$gsil'");}
						if($temizle)
			...
			if(isset($_POST["genelguncelle"])){
				$gguncelbaslik=mysql_real_escape_string($_POST["gguncelbaslik"]);
				$gguncelicerik=mysql_real_escape_string($_POST["gguncelicerik"]);
